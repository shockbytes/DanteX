...
import 'package:dantex/com.shockbytes.dante/data/bookdownload/entity/remote/GoogleBooksResponse.dart';
...
class GoogleBooksApi implements BookApi {
  ...
  Future<BookSuggestion> downloadBook(String query) {
    return _dio.get(
      ...
      GoogleBooksResponse parsedResponse = GoogleBooksResponse.fromJson(response.data);
  ...
  Book _fromItem(Items item) {
    ...
    return Book(
      ...
      position: 0,
      isbn: volumeInfo.industryIdentifiers?.where((ii) => ii.type == 'ISBN_13').map((e) => e.identifier!).join(', ') ?? '',
